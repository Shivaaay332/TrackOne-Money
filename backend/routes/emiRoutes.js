...
// 🔥 MAGIC: AUTOMATIC EMI PAYMENT LOGIC WITH HISTORY 🔥
router.post('/:id/pay', protect, async (req, res) => {
    try {
        ...
        // 🔥 FIX: Next Due Date ko exactly 1 Mahina aage badhao 🔥
        if (emi.nextDueDate) {
            const nextDate = new Date(emi.nextDueDate);
            nextDate.setMonth(nextDate.getMonth() + 1); // Mahina +1 kar diya
        ...
        // 🔥 Auto-Generate History Log (Jis date ko payment hua wo aaj ki date mein save hoga)
        await HistoryLog.create({
            ...
            actionType: 'Paid',
            amount: amountPaid,
            date: new Date(), // Yeh aaj ki current date set karega history ke liye
